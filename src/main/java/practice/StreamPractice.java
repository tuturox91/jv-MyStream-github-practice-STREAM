...
public class StreamPractice {
    ...
    private final static int SUBTRACT_NUMBER = 1;
    /**
    ...
    public int findMinEvenNumber(List<String> numbers) {
        return numbers.stream()
                .flatMap(w -> Arrays.stream(w.split(SPLITTER)))
    ...
    public Double getOddNumsAverage(List<Integer> numbers) {
        return IntStream.range(0, numbers.size())
                .map(i -> i % 2 != 0 ? numbers.get(i) - SUBTRACT_NUMBER : numbers.get(i))
                .filter(i -> i % 2 != 0)
                .average()
                .orElseThrow();
    }
    ...
    public List<Person> getWorkablePeople(int fromAge, int femaleToAge, int maleToAge, List<Person> peopleList) {
        Predicate<Person> predicate = person -> person.getAge() >= fromAge && person.getAge() <= femaleToAge
                || person.getSex() == Person.Sex.MAN && person.getAge() >= fromAge && person.getAge() <= maleToAge;
